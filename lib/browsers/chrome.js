...
chrome.spawn = function(options) {
	return new Promise((resolve, reject) => {
        ...
        let chromeFlags = ['--headless', '--disable-gpu', '--hide-scrollbars'];

        let remoteDebuggingPort = this.options.remoteDebuggingPort || 9222;
		chromeFlags.push('--remote-debugging-port=' + remoteDebuggingPort);

		this.chromeChild = spawn(location, this.options.chromeFlags || chromeFlags);
	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			...
			if (this.options.remoteDebuggingPort) {
				cdpOptions.port = this.options.remoteDebuggingPort;
			}

			CDP.Version(cdpOptions).then((info) => {
        		let remoteDebuggingPort = this.options.remoteDebuggingPort || 9222;
        		let webSocketDebuggerUrl = 'ws://localhost:' + remoteDebuggingPort + '/devtools/browser';

				this.originalUserAgent = info['User-Agent'];
				this.webSocketDebuggerURL = info.webSocketDebuggerUrl || webSocketDebuggerUrl;
...
chrome.openTab = function(options) {
	return new Promise((resolve, reject) => {
		...
		.then((chromeBrowser) => {
			...
			return CDP({ target: targetId, port: this.options.remoteDebuggingPort });
		}).then((tab) => {
